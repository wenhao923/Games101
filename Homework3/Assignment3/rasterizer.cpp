...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    {
        //std::cout<<count<<std::endl;  第几个三角形
        count++;
        Triangle newtri = *t;
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    auto v = t.toVector4();
    //三角形的包围盒
    float top=0,bottom=MAXFLOAT,left=MAXFLOAT,right=0;
    for (auto & vert : v)
    {
        ...
        //std::cout<<"("<<left<<","<<bottom<<")"<<"   "<<"("<<right<<","<<top<<")"<<std::endl;
    }
    ...
    for (int y=bottom;y<=top;y++) {
        ...
        {            
            bool temp=insideTriangle(x,y,t.v);//判断当前像素在不在三角形内
